...
if (!$cddoacao and (!$dtrecini or !$dtrecfim)) {
	$msg = 'Favor informar código do recibo ou período de recebimento';
	$erro = true;
}
...
<html>
	...
	<body style="margin:0; padding:0;">
		...
		while ($res = mysql_fetch_assoc($sql)) {
			...
			if ($dtniverresp == $dtaux or $dtniverconjuge == $dtaux) {
				$res['obsrecibo'] .= '  <strong>***FELIZ ANIVERSÁRIO!***</strong>';
			}
					...
					<tr>
						<td style="width:100%; text-align:center;">
							<table border="0" style="border:2px solid; height:400px; padding:10px 15px; width:100%;">
								<tr>
									<td align="center" colspan="2"><strong style="font-size:14px;">ASSOCIAÇÃO DE PAIS E AMIGOS DOS EXCEPCIONAIS DE IPATINGA</strong></td>
								</tr>
								<tr>
									...
									<td style="padding-left:15px; text-align:center; font-size:12px;">
										...
										CNPJ 20.951.190/0001-30 - U.P. MUNICIPAL - LEI Nº 649 de 19/07/79<br />
										U.P. ESTADUAL - LEI Nº 7656 de 27/12/79 - U.P. FEDERAL DECRETO LEI Nº 91.108 de 12/03/85<br />
										Registro CNSS: 23.002.006747/88-53 - TELEFONE: (31)3822-3502
									</td>
								</tr>
								<tr><td colspan="2" style="font-size:18px; font-weight: bold; text-align:center;">RECIBO Nº ' . number_format($res['cddoacao'], 0, '', '.') . ' </td></tr>
								<tr>
									<td colspan="2" style="line-height:20px; font-size:14px;">
									Recebemos de <strong style="font-size:14px;">' . $res['nmresponsavel'] . '</strong>, ' . $cpf . ' residente na ' . $res['endereco'] . ', <strong style="font-size:14px;">' . $res['nmbairro'] . '</strong>, ' . $res['nmcidade'] . '/' . $res['sgestado'] . $tel . ', a quantia de <strong style="font-size:14px;">' . $vldoacao . '</strong>, referente à doação do dia <strong style="font-size:14px;">' . inverte_formato_data($res['dtrec'], '/') . '</strong>.
									<br /><br />
